...
The following example shows how to use `go:generate` to generate Sloth SLO specification for a simple application.
...
```go
    var (
        ...
        metricTenantTotalLoginsCount = prometheus.NewGauge(
...
Running `go generate ./...` in the terminal, will run `slotalk` which will parse the different packages for in code annotations and
generate Sloth SLOs Specification, under `./slo_definitions/chatgpt.yaml`.
